...
const FILES_API_POLL_TIMEOUT_MS = 60000
...
/**
 * URL의 오디오를 받아쓰기.
 * @param {string} url - 노션의 signed audio URL
 * @param {object} [opts]
...
export async function transcribeAudioFromUrl(url, opts = {}) {
  ...
  const mimeType = opts.mimeType || detectAudioMime(url)

  // 2. 받아쓰기 (사이즈에 따라 inline 또는 Files API)
  let text
  let mode
  ...
  if (!text) {
    throw new Error('Gemini 응답이 비어있습니다 (받아쓰기 결과 0자).')
  }
