...
const MAX_DEPTH = 6           // 너무 깊으면 시간 폭발
const MAX_BLOCKS = 2000        // 페이지당 블록 한도(이전 5000 → 2000)
const TOTAL_TIMEOUT_MS = 75000 // 전체 fetch 타임아웃 (75초)
  ...
  try {
    const work = (async () => {
      const [t, blocks] = await Promise.all([
        ...
        fetchBlocksTree(client, pageId, state, 0),
      ])
      return { t, blocks }
    })()
