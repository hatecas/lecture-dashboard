...
import { planThumbnail } from './thumbnail'
...
export const PLANNER_META = {
  ...
  alimtalk: {
    label: '채널톡 멘트',
    icon: '💬',
    description: '인입/후속/리마인드 시나리오 3종 (알림톡은 템플릿 별도)',
    enabled: true,
  },
  viralQ: {
    label: '바이럴 질문',
  ...
  groupAnnouncement: {
    label: '단톡방 입장시 필독 공지',
    icon: '📢',
    description: '신규 입장자가 처음 보는 공지 (N잡연구소 표준 양식)',
    enabled: true,
  },
  thumbnail: {
    label: '전자책/강의 썸네일 기획',
    icon: '🖼️',
    description: '4가지 디자인 타입(TEXT_HERO/BOOK_COVER/OBJECT_MOTIF/DARK_MONEY)별 합성용 명세 + 미리보기',
    enabled: true,
  },
}
