...
export async function planThumbnail(ctx) {
  ...
  // system 배열 구성:
  //   1) 헤더 텍스트 (캐시)
  //   2) 각 레퍼런스마다 [라벨 텍스트 + 이미지] 두 블록씩 (캐시)
  //   3) 푸터 텍스트 — 지침/출력 형식 (캐시)
  // 모두 cache_control: ephemeral → 5분 내 재호출 시 90% 할인.
  const systemBlocks = [
    { type: 'text', text: buildSystemTextHeader() },
  ]
  for (const { text, image } of imageBlocks) {
    systemBlocks.push({ type: 'text', text })
    systemBlocks.push(image)
  }
  systemBlocks.push({
    type: 'text',
    text: buildSystemTextFooter({ instructions }),
    cache_control: { type: 'ephemeral' }, // 마지막 블록에만 걸어도 전체가 캐시됨
  })
...
위 자료와 학습한 8장 레퍼런스를 종합해 1080×1620 세로 썸네일 명세 JSON만 출력하세요.`
