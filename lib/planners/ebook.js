// 무료 전자책 기획안 생성 봇.
// 사용자가 제공할 실제 레퍼런스/지침으로 교체될 자리는 // [TODO:USER] 마커.
// 출력은 JSON으로 강제해 프론트가 구조화 매핑할 수 있게 함.

// [TODO:USER] 사용자가 제공할 레퍼런스 모범 사례. 지금은 1개 placeholder.
const REFERENCES = `
=== 레퍼런스 1: 청담언니 루시 - 월 2,500만 원 번 유튜브 수익화 설계도 ===
"유튜브, 영상 잘 만드는 사람이 돈 벌까요? 아니요, '설계' 잘하는 사람이 법니다"

"열심히"가 배신하는 시장, 유튜브
유튜브 AI 부업 강의를 한 번이라도 들어본 사람은 압니다. 챗GPT로 대본 쓰고, 다른 AI로 이미지 만들고, 캡컷으로 편집하고. 툴 두세 개를 왔다 갔다 하다 보면 어느새 다섯 시간이 사라집니다.
말이 좋아 자동화지, 실제로는 사람이 일일이 옮기고 붙여넣는 노가다입니다. 더 큰 문제는 따로 있습니다. 이 방식으로 대량 생성만 하다가 수개월 키운 채널이 스팸으로 분류돼 한 방에 날아가는 사례가 꾸준히 나옵니다. 열심히 한 것과 잘 한 것이 전혀 다른 결과를 낳는 시장. 그게 지금의 유튜브입니다.

1. 열심히 하는 것과 잘 설계하는 것
많은 사람이 유튜브를 '영상 올리는 곳'으로 생각합니다. 틀렸습니다. 유튜브는 검색 엔진입니다.

2. 툴 3개 켜는 동안, 저는 이미 영상을 올렸습니다
솔직하게 말씀드릴게요. 챗GPT, 미드저니, 캡컷. 저도 다 써봤습니다. 그리고 결론을 냈습니다.

3. "진짜 수익은 '노동'이 멈출 때 시작됩니다"
많은 AI 유튜버들이 '제작'에만 매몰될 때, 우리는 '수익 구조'를 고민해야 합니다.
...
CTA: "더 자세한 청담언니 루시의 유튜브 수익화 전략이 궁금하시나요? 전자책 받으러 가기"
`

// [TODO:USER] 사용자가 다듬을 작성 지침. 지금은 1차 초안.
const INSTRUCTIONS = `
- 톤: 강사가 직접 1인칭으로 말하는 느낌. 권위 + 진정성. 광고 멘트 X.
...
- CTA: "전자책 받으러 가기" + 무료 라이브 강의 안내 (날짜는 비워둠 → 운영자가 채움)
- 금지: "꼭 보세요!", "지금 바로!" 같은 닳은 표현
- 길이: 전체 800~1500자
`

const SYSTEM_PROMPT = `당신은 N잡연구소의 무료 전자책 기획 전문가입니다. 강사가 무료 전자책으로 수강생을 모집할 때 쓸 상세페이지의 카피를 작성합니다.
...
위 강사의 톤으로, 작성 지침과 출력 형식을 따라 JSON만 출력하세요.`
  ...
  const response = await fetch(ANTHROPIC_ENDPOINT, {
    ...
    body: JSON.stringify({
      model: MODEL,
      max_tokens: 4096,
      // 시스템 프롬프트는 매 호출 동일 → 캐시. 5분 내 재호출 시 90% 할인.
      system: [
        {
          type: 'text',
          text: SYSTEM_PROMPT,
  ...
  // 가끔 모델이 ```json ... ``` 코드블록을 붙이면 안에서 JSON 추출
  ...
  return { plan: parsed, usage, model: MODEL }
}
