...
export async function sendSMS(phoneNumber, text) {
  ...
  try {
    ...
    console.log('[SMS] 발송 시도:', { to: cleanPhone, from: fromNumber, apiKey: apiKey.substring(0, 4) + '...' })
    ...
    const payload = {
      message: {
        ...
        from: fromNumber,
        text: text,
        type: new TextEncoder().encode(text).length > 90 ? 'LMS' : 'SMS'
      }
    }
    ...
    console.log('[SMS] Solapi 응답:', res.status, JSON.stringify(data))
    ...
    console.error('[SMS] 에러:', e.message)
...
export async function sendNotification({ task, recipient, triggerReason }) {
  ...
  const [smsResult, slackResult] = await Promise.allSettled([
    recipient.phone ? sendSMS(recipient.phone, smsText) : Promise.resolve(null),
    recipient.slack_email ? sendSlackDM(recipient.slack_email, slackText) : Promise.resolve(null)
  ])
  results.sms = smsResult.status === 'fulfilled' ? smsResult.value : { success: false, error: smsResult.reason?.message }
  results.slack = slackResult.status === 'fulfilled' ? slackResult.value : { success: false, error: slackResult.reason?.message }
